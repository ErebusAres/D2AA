(() => {
  ...
  function ensureModal() {
    ...
    modal.innerHTML = `
      <div class="d2aa-compare-backdrop" data-compare-close="1"></div>
      <section class="d2aa-compare-panel" role="dialog" aria-modal="true" aria-labelledby="d2aaCompareTitle">
        <header class="d2aa-compare-head">
          <div><p class="eyebrow">Duplicate group</p><h2 id="d2aaCompareTitle">Compare Group</h2></div>
          <button class="d2aa-compare-close" type="button" data-compare-close="1" aria-label="Close comparison">×</button>
        </header>
        <div class="d2aa-compare-body" id="d2aaCompareBody"></div>
      </section>`;
    modal.addEventListener('click', (event) => {
      ...
      if (copyBtn) {
        ...
        const ids = rows.map((item) => `id:${normId(item.Id)}`).join(' or ');
  ...
  function itemCard(row, bestByStat) {
    ...
    return `<article class="d2aa-compare-item" data-compare-id="${esc(normId(row.Id))}">
      <div class="d2aa-compare-item-top">
        <div class="d2aa-compare-icon">${icon ? `<img src="${esc(icon)}" alt="" loading="lazy">` : `<span>${esc(slotLabel(row.Type).slice(0, 1))}</span>`}</div>
        <div class="d2aa-compare-title"><strong>${esc(row.Name || '(Unnamed item)')}</strong><span>${esc(row.Equippable || '')} • ${esc(slotLabel(row.Type))} • ${esc(row.Rarity || '')}</span></div>
        <div class="d2aa-compare-tag-display">${tag}</div>
      </div>
      <div class="d2aa-compare-summary"><span title="Base stat total">${num(row['Total (Base)'])}</span><span title="Gear tier">${diamonds(row)}</span><span>${esc(row.Source || '')}</span></div>
      <div class="d2aa-compare-stats">
        ${STAT_COLS.map((stat, index) => {
          const val = num(row[stat]);
          const isBest = bestByStat[stat] === val && val > 0;
          return `<div class="d2aa-compare-stat${isBest ? ' is-best' : ''}"><span>${STAT_LABELS[index]}</span><strong>${val}</strong></div>`;
        }).join('')}
      </div>
      <div class="d2aa-compare-tags" aria-label="Change tag for ${esc(row.Name || 'item')}">
        ${TAGS.map((tagOpt) => `<button type="button" data-compare-tag="${tagOpt.id}" class="${String(row.Tag || '') === tagOpt.id ? 'is-active' : ''}" title="${tagOpt.label}">${tagOpt.icon}</button>`).join('')}
      </div>
    </article>`;
  }
  ...
  function openModal(row) {
    ...
    body.innerHTML = `
      <div class="d2aa-compare-tools">
        <span>${rows.length} matching items</span>
        <button type="button" data-compare-copy="1">Copy group IDs</button>
      </div>
      <div class="d2aa-compare-grid">${rows.map((item) => itemCard(item, bestByStat)).join('')}</div>`;
