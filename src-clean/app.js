...
import { renderGrid } from './render/grid-v164.js';
...
function normalizeTheme(theme) { return THEME_NAMES.includes(theme) ? theme : 'calus'; }
function toggleOptionsPanel() { setOptionsPanelOpen(!els.commandPanel?.classList.contains('is-open')); }
function setOptionsPanelOpen(open) { els.commandPanel?.classList.toggle('is-open', open); document.body.classList.toggle('options-open', open); els.menuBtn?.setAttribute('aria-expanded', String(open)); }
function closeOptionsOnOutsideClick(event) { if (!document.body.classList.contains('options-open')) return; if (els.commandPanel?.contains(event.target) || els.menuBtn?.contains(event.target)) return; setOptionsPanelOpen(false); }
function closeOptionsOnEscape(event) { if (event.key === 'Escape') setOptionsPanelOpen(false); }
function setClassFilter(className) { setState({ filters: { ...state.filters, class: normalizeClassFilter(className) } }); }
function restoreFeedOpenState() { const open = readJson(STORAGE_KEYS.feedOpen, false) === true; els.itemFeed?.classList.toggle('is-open', open); document.body.classList.toggle('feed-open', open); }
...
function render() {
  ...
  syncViewButtons(); syncThemeButtons(); syncFilterOptions(); syncClassToggle();
...
async function handleCardAction(actionId, button) {
  ...
  } finally { if (button) setTimeout(() => { button.textContent = original; }, 1200); }
}
function openGroupCompare(groupKey) { const rows = lastGroupedRows.filter((row) => row.Is_Dupe && row.GroupActionKey === groupKey); if (!rows.length) return setStatus('No duplicate group found to compare.'); openCompareModal(rows, { onTag: updateTag, onPullGroup: (groupRows, button) => pullGroup(groupRows[0]?.GroupActionKey || groupKey, button) }); }
async function pullGroup(groupKey, button) {
  ...
  try { const result = await runGroupPull(rows); setStatus(result.message || 'Group action complete.'); if (button) button.textContent = 'Done'; if (result.needsRefresh) requestBungieRefresh('post-group-pull-refresh'); }
  catch (error) { console.error('D2AA clean group pull failed', error); setStatus(error.message || String(error)); if (button) button.textContent = 'Failed'; }
  finally { if (button) setTimeout(() => { button.textContent = original; }, 1400); }
}
...
function syncFilterOptions() { fillSelect(els.classFilter, ['all', ...CLASS_ORDER.filter((cls) => state.rows.some((row) => rowMatchesClass(row, cls)))], normalizeClassFilter(state.filters.class)); fillSelect(els.slotFilter, ['all', ...SLOT_ORDER.filter((slot) => state.rows.some((r) => r.Slot === slot)), ...unique(state.rows.map((r) => r.Slot)).filter((slot) => !SLOT_ORDER.includes(slot))], state.filters.slot); fillSelect(els.rarityFilter, ['all', ...unique(state.rows.map((r) => r.Rarity))], state.filters.rarity); }
function fillSelect(select, values, selected) { if (!select) return; const current = select.value || selected; select.innerHTML = values.map((value) => `<option value="${html(value)}">${value === 'all' ? 'All' : html(value)}</option>`).join(''); select.value = values.includes(selected) ? selected : values.includes(current) ? current : 'all'; }
function renderChips() { const chips = []; if (state.search) chips.push(`Search: ${state.search}`); Object.entries(state.filters).forEach(([key, value]) => { if (key === 'class') return; if (value !== 'all') chips.push(`${key}: ${value}`); }); if (state.duplicateTolerance !== 5) chips.push(`Tolerance: ±${state.duplicateTolerance}`); els.activeChips.innerHTML = chips.map((chip) => `<span>${html(chip)}</span>`).join(''); }
function unique(values) { return [...new Set(values.filter(Boolean))].sort(); }
function countByClass(rows) { return CLASS_ORDER.reduce((acc, cls) => { acc[cls] = rows.filter((row) => rowMatchesClass(row, cls)).length; return acc; }, {}); }
