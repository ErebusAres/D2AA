...
function emptyBreakdown() { const out = Object.fromEntries(BONUS_TYPES.map((type) => [type, emptyArmorStats()])); out._unknownMasterwork = 0; return out; }
function totalOf(row) { return STAT_KEYS.reduce((sum, key) => sum + Number(row?.[key] || 0), 0); }
function absoluteTotalOf(row) { return STAT_KEYS.reduce((sum, key) => sum + Math.abs(Number(row?.[key] || 0)), 0); }
...
function cloneBreakdown(breakdown) { const out = emptyBreakdown(); for (const type of BONUS_TYPES) for (const key of STAT_KEYS) out[type][key] = Number(breakdown?.[type]?.[key] || 0); out._unknownMasterwork = Number(breakdown?._unknownMasterwork || 0); return out; }
...
function isSubtractableArmorBonusPlug(plugDef) { const name = normalizeName(plugDef?.displayProperties?.name); const desc = normalizeName(plugDef?.displayProperties?.description); const type = normalizeName(plugDef?.itemTypeDisplayName); const category = normalizeName(plugDef?.plug?.plugCategoryIdentifier); const hasStats = Array.isArray(plugDef?.investmentStats) && plugDef.investmentStats.some((s) => Number(s.value || 0) !== 0); if (!hasStats) return false; const isKnownStatBonus = type.includes('armor mod') || category.includes('armor mods') || category.includes('masterwork') || name.includes('masterwork') || desc.includes('masterwork') || name.includes('artifice') || desc.includes('artifice') || category.includes('artifice'); const isGenericStatMod = name.includes('mod') && (desc.includes('stat') || desc.includes('bonus') || desc.includes('increase') || desc.includes('reduced') || desc.includes('penalty')); return isKnownStatBonus || isGenericStatMod; }
function signedStatsForPlug(plugDef, statColumnMap) { const raw = emptyArmorStats(); for (const stat of plugDef?.investmentStats || []) { const column = statColumnMap[toUint32(stat.statTypeHash)] || statColumnMap[toSigned32(stat.statTypeHash)]; const value = Number(stat.value || 0); if (column && value !== 0) raw[column] += value; } return raw; }
function positiveStatsOnly(stats) { const out = emptyArmorStats(); for (const key of STAT_KEYS) out[key] = Math.max(0, Number(stats?.[key] || 0)); return out; }
function masterworkBonusForPlug(plugDef, statColumnMap) { const rawSigned = signedStatsForPlug(plugDef, statColumnMap); const raw = positiveStatsOnly(rawSigned); const rawTotal = totalOf(raw); if (!rawTotal || rawTotal <= 10) return raw; const target = columnFromPlugText(plugDef); const fixed = emptyArmorStats(); if (target) fixed[target] = 10; else fixed.__unknownMasterwork = 10; return fixed; }
function socketBonusBreakdown(activePlugDefs, statColumnMap) { const breakdown = emptyBreakdown(); for (const plugDef of activePlugDefs || []) { if (!isSubtractableArmorBonusPlug(plugDef)) continue; const type = bonusTypeForPlug(plugDef); if (type === 'masterwork') { const mw = masterworkBonusForPlug(plugDef, statColumnMap); breakdown._unknownMasterwork += Number(mw.__unknownMasterwork || 0); for (const key of STAT_KEYS) breakdown.masterwork[key] += Number(mw[key] || 0); continue; } const raw = signedStatsForPlug(plugDef, statColumnMap); for (const key of STAT_KEYS) breakdown[type][key] += Number(raw[key] || 0); } if (totalOf(breakdown.masterwork) > 10) { const target = STAT_KEYS.reduce((best, key) => breakdown.masterwork[key] > breakdown.masterwork[best] ? key : best, STAT_KEYS[0]); breakdown.masterwork = emptyArmorStats(); breakdown.masterwork[target] = 10; } return breakdown; }
function reconcileUnknownMasterwork(current, breakdown, socketTotals) { const unknown = Math.min(10, Number(breakdown?._unknownMasterwork || 0)); if (!unknown) return; const nonMw = emptyArmorStats(); for (const type of BONUS_TYPES.filter((t) => t !== 'masterwork')) for (const key of STAT_KEYS) nonMw[key] += Number(breakdown?.[type]?.[key] || 0); const currentTotal = totalOf(current); const knownNonMwTotal = totalOf(nonMw); const needed = Math.min(unknown, Math.max(0, currentTotal - knownNonMwTotal - 75)); if (!needed) return; const candidates = STAT_KEYS.filter((key) => Number(current[key] || 0) - Number(nonMw[key] || 0) >= needed); const preference = ['ClassAbility', 'Grenade', 'Melee', 'Super', 'Weapon', 'Health']; const target = preference.find((key) => candidates.includes(key)) || candidates[0]; if (!target) return; breakdown.masterwork[target] += needed; socketTotals[target] += needed; breakdown._unknownMasterwork -= needed; }
function statsForItem(def, statComponent, bonusBreakdown, statColumnMap) { const current = emptyArmorStats(); const source = Object.keys(statComponent?.stats || {}).length ? statComponent.stats : (def.stats?.stats || {}); for (const [hash, stat] of Object.entries(source || {})) { const column = statColumnMap[toUint32(hash)] || statColumnMap[toSigned32(hash)] || HASH_TO_COLUMN[toUint32(hash)] || HASH_TO_COLUMN[toSigned32(hash)]; if (column) current[column] = getStatNumericValue(stat); } const safeInput = cloneBreakdown(bonusBreakdown); const socketTotals = emptyArmorStats(); for (const type of BONUS_TYPES) for (const key of STAT_KEYS) socketTotals[key] += Number(safeInput?.[type]?.[key] || 0); reconcileUnknownMasterwork(current, safeInput, socketTotals); const suspicious = absoluteTotalOf(socketTotals) > 45 || STAT_KEYS.some((key) => Number(socketTotals[key] || 0) > Number(current[key] || 0) + 10); const safeBreakdown = suspicious ? emptyBreakdown() : safeInput; if (suspicious) for (const key of STAT_KEYS) socketTotals[key] = 0; const base = emptyArmorStats(); for (const key of STAT_KEYS) base[key] = Math.max(0, current[key] - Number(socketTotals[key] || 0)); const row = { ...base }; row.Total = totalOf(base); row.BaseTotal = row.Total; row.CurrentTotal = totalOf(current); row.StatBonusTotal = row.CurrentTotal - row.BaseTotal; row.StatSource = suspicious ? 'BungieInstanceBonusGuarded' : Object.keys(statComponent?.stats || {}).length ? 'BungieInstanceMinusSignedActiveSocketAdjustments' : 'DefinitionFallbackMinusSignedActiveSocketAdjustments'; for (const key of STAT_KEYS) { row[`Base${key}`] = base[key]; row[`Current${key}`] = current[key]; row[`StatBonus${key}`] = Number(socketTotals[key] || 0); for (const type of BONUS_TYPES) row[`${title(type)}Bonus${key}`] = Number(safeBreakdown?.[type]?.[key] || 0); } for (const type of BONUS_TYPES) row[`${title(type)}BonusTotal`] = STAT_KEYS.reduce((sum, key) => sum + Number(row[`${title(type)}Bonus${key}`] || 0), 0); return row; }
