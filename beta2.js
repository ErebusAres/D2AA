...
function clusterRows(filtered) {
  const byKey = new Map();
  for (const row of filtered) { const key = row.Rarity === 'Exotic' ? `${row.Type}|${normName(row.Name)}` : row.Type; if (!byKey.has(key)) byKey.set(key, []); byKey.get(key).push({ ...row }); }
  const out = []; const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  for (const [key, items] of byKey) {
    const assigned = Array(items.length).fill(false); const groups = [];
    for (let i = 0; i < items.length; i++) { if (assigned[i]) continue; const group = [i]; assigned[i] = true; for (let j = i + 1; j < items.length; j++) { if (assigned[j]) continue; if (similarTop3(items[i], items[j], STATE.tol)) { group.push(j); assigned[j] = true; } } groups.push(group); }
    groups.sort((a, b) => Math.max(...b.map((i) => num(items[i]['Total (Base)']))) - Math.max(...a.map((i) => num(items[i]['Total (Base)']))));
    let letterIdx = 0; const isExoticGroupKey = key.includes('|');
    for (const group of groups) { const first = items[group[0]]; const label = group.length > 1 ? `${slotNumber(first.Type)}${letters[Math.min(letterIdx++, letters.length - 1)]}` : 'X'; for (const idx of group) { const item = items[idx]; const rank = item.Rarity === 'Exotic' ? exoticRank(item['Total (Base)']) : legendaryRank(item['Total (Base)']); out.push({ ...item, GroupKey: key, Dupe_Group: label, Rank: rank, RankScore: rankScore(rank), Is_Dupe: label !== 'X', Is_Dupe_Exotic: isExoticGroupKey && label !== 'X' }); } }
  }
  out.sort(defaultSort); return out;
}
...
function renderGroupBadge(row, groupRows) {
  ...
  const btn = document.createElement('button'); btn.type = 'button'; btn.className = 'group-badge'; btn.title = isBungieMode() ? 'Pull all items in this duplicate group to matching character inventory' : 'Copy all DIM IDs in this duplicate group'; btn.append('⚠️'); if (row.Is_Dupe_Exotic) btn.appendChild(iconImg(RARITY_ICONS.Exotic, 'Exotic duplicate group', 'badge-icon')); btn.append(row.Dupe_Group);
  btn.addEventListener('click', async () => { try { await copyOrPullGroup(row, groupRows, btn); } catch (err) { console.error(err); btn.textContent = 'Failed'; alert(err.message || err); } setTimeout(() => render(), 1200); });
