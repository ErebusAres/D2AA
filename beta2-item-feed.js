(() => {
  ...
  function saveFeed(items) { writeJson(LS_FEED, (items || []).slice(0, MAX_FEED)); updateLauncherCount(); }
  ...
  function tagEmoji(tag) { return (TAGS.find(([value]) => value === String(tag || '').toLowerCase()) || TAGS[0])[1]; }
  ...
  function buildFeedEntry(row) {
    return { id: normId(row.Id), name: row.Name || 'Unknown Armor', type: row.Type || 'Armor', rarity: row.Rarity || 'Unknown', equippable: row.Equippable || '', total: Number(row['Total (Base)'] || 0), location: locationLabel(row), icon: locationIcon(row), tag: normalizeTag(row.Tag), foundAt: new Date().toISOString() };
  }
  ...
  function renderFeed() {
    ...
    for (const item of feed.slice(0, MAX_FEED)) {
      ...
      card.innerHTML = `<div class="feed-title"><span>✨</span><span>${esc(item.name)}</span></div><div class="feed-meta">${esc(item.icon || '')} ${esc(item.location || 'Unknown')} • ${esc(item.type || 'Armor')} • ${esc(item.equippable || '')} • <span class="feed-total">${Number(item.total || 0)}</span></div><div class="feed-meta">Found ${esc(when)}</div>`;
  ...
  function ensurePanel() {
    ...
    drawer.innerHTML = `<div class="item-feed-head"><div><h2 id="itemFeedTitle">Item Feed</h2><p class="muted compact">New armor since your previous Bungie sync. Assign tags here or use the table tag cell.</p></div><button id="itemFeedClose" class="item-feed-close" type="button" aria-label="Close item feed">×</button></div><div class="item-feed-actions"><button id="showRecentBtn" class="mini-feed-btn" type="button">Show recent</button><button id="refreshFeedBtn" class="mini-feed-btn" type="button">Refresh</button><button id="clearFeedBtn" class="mini-feed-btn" type="button">Clear</button></div><div id="itemFeedList" class="item-feed-list" aria-live="polite"></div>`;
