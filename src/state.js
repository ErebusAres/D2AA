(() => {
  ...
  function saveRows() { writeJson(STORAGE.rows, state.rows); }
  ...
  function setRows(rows, label = '') { state.rows = Array.isArray(rows) ? rows.map((row) => ({ ...row, Tag: tagFor(row.Id) || row.Tag || '' })) : []; applyFilters(); saveRows(); if (label) window.D2AA_RENDER?.setStatus?.(label); }
  function setView(view) { state.view = view === 'table' ? 'table' : 'grid'; localStorage.setItem(STORAGE.view, state.view); render(); }
  function setTag(rowOrId, tag) { const id = norm(typeof rowOrId === 'object' ? rowOrId.Id : rowOrId); if (!id) return; const clean = norm(tag).toLowerCase(); if (clean) state.tags[id] = clean; else delete state.tags[id]; writeJson(STORAGE.tags, state.tags); state.rows = state.rows.map((row) => norm(row.Id) === id ? { ...row, Tag: clean } : row); applyFilters(); }
  function rowText(row) { return [row.Name,row.Type,row.Rarity,row.Equippable,row.Dupe_Group,row.Tag,row.Light,row.Power].join(' ').toLowerCase(); }
