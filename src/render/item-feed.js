(() => {
  ...
  function renderFeed() { const list = document.getElementById('itemFeedList'); const count = document.getElementById('itemFeedCount'); if (!list) return; const items = feed(); const signature = JSON.stringify(items.map((item) => [item.id,item.name,item.total,item.light,item.icon])); if (count && count.textContent !== String(items.length)) count.textContent = String(items.length); if (signature === lastFeedSignature) return; lastFeedSignature = signature; list.innerHTML = items.length ? items.map((item) => `<div class="feed-card">${item.icon ? `<img src="${esc(item.icon)}" alt="" loading="lazy">` : '<div></div>'}<div><div class="feed-title">${esc(item.name)}</div><div class="feed-meta">${esc(item.rarity)} • ${esc(item.slot)}${item.light ? ` • ${esc(item.light)}` : ''}</div></div><div class="feed-total">${Number(item.total || 0)}</div></div>`).join('') : '<p class="feed-empty">No recent items yet. Import a DIM CSV to seed the feed.</p>'; }
