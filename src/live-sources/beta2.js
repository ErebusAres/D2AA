...
function parseRows(data) { return data.map((row) => { const x = { ...row }; for (const key of [...STAT_COLS, 'Total (Base)', 'Tier', 'GearTier']) x[key] = Number.isFinite(num(x[key])) ? num(x[key]) : 0; x.Id = normId(x.Id); return applyTagOverrides(x); }).filter((row) => row.Id && row.Name && row.Type && row.Equippable); }
