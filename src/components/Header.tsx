...
export default function Header({ status, auth, filters, allRows, onFiltersChange, onOptionsToggle, onSync, isSyncing, liveEnabled, lastSyncAt, queuedActions, runningActionLabel = '' }: HeaderProps) {
  const liveLabel = queuedActions ? `Actions ${queuedActions}` : isSyncing ? 'Syncing' : liveEnabled ? 'Live' : 'Manual';
  ...
  return (
    <header className="command-bar">
      ...
      <div className={`live-chip ${isSyncing ? 'is-syncing' : ''} ${queuedActions ? 'is-queued' : ''} ${liveEnabled ? 'is-live' : 'is-manual'}`} data-live-state={liveState} title={queuedActions ? `${queuedActions} Bungie action${queuedActions === 1 ? '' : 's'} active or queued${runningActionLabel ? `: ${runningActionLabel}` : ''}. ${lastSyncLabel}` : lastSyncLabel}><span />{liveLabel}</div>
