(() => {
  ...
  function lightFrom(row) {
    const candidates = ['Power','Power Level','PowerLevel','Light','Light Level','PrimaryStat','Primary Stat'];
  ...
  function gearTierFrom(row, total) {
    const direct = num(first(row, ['GearTier','Gear Tier','Armor Tier','D2AA Gear Tier']));
    if (direct >= 1 && direct <= 5) return { tier: direct, source: 'DIM' };
    if (total >= 73) return { tier: 5, source: 'Fallback' };
    if (total >= 65) return { tier: 4, source: 'Fallback' };
    if (total >= 59) return { tier: 3, source: 'Fallback' };
    if (total >= 54) return { tier: 2, source: 'Fallback' };
    return { tier: total ? 1 : 0, source: total ? 'Fallback' : '' };
  }
  ...
  function normalizeArmorRow(raw, index = 0) { const row = { __raw: raw };
    ...
    const tierInfo = gearTierFrom(raw, row['Total (Base)']);
    row.Tier = tierInfo.tier;
    row.GearTier = tierInfo.tier;
    row.TierSource = tierInfo.source;
    row.TierMax = 5;
    row.Source = 'DIM';
