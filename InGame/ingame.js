...
function bonusParts(row,key){ const fallback=n(row[`StatBonus${key}`]); const parts=BONUS_ORDER.map(type=>({type,value:n(row[`${cap(type)}Bonus${key}`])})).filter(p=>p.value>0); const known=parts.reduce((t,p)=>t+p.value,0); if(fallback>known) parts.push({type:'other',value:fallback-known}); if(!parts.length && fallback>0) parts.push({type:'other',value:fallback}); return parts; }
