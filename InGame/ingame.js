...
const els = {};
let lastGroupedRows = [];
const GRADE_ORDER = { S: 6, A: 5, B: 4, C: 3, D: 2, F: 1 };
...
function renderCard(row){
  const loc=locationText(row); const tag=row.Tag||''; const grade=gradeFor(row);
  return `<article class="armor-card rarity-${rarityClass(row.Rarity)}" data-id="${h(row.Id)}" data-group="${h(row.GroupActionKey||'')}">
    <div class="card-title"><img class="item-icon" src="${h(row.IconUrl||row.Icon||'')}" alt=""/><div class="tier-rail">${tierMarks(row).join('')}</div><div class="title-copy"><strong title="${h(row.Name)}">${h(row.Name)}</strong><div class="meta-line"><button type="button" class="tag-chip" data-tag-button data-id="${h(row.Id)}">${tag?tagLabel(tag):'+'}</button><span class="location-chip">${h(loc)}</span><span class="grade-chip grade-${grade.letter}">${grade.letter}</span></div></div>${row.Is_Dupe?`<button type="button" class="group-badge" data-compare-group="${h(row.GroupActionKey)}" title="Compare group">${h(row.Group_Label||row.GroupKey||'1A')}</button>`:''}<div class="power-badge">${h(row.Power||row.Light||'')}</div></div>
