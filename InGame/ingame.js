...
function renderGrid(rows){
  ...
  els.gridView.innerHTML=bySlot.map(([slot,items])=>{ const collapsed=collapsedSlots.has(slot); return `<section class="slot-section ${collapsed?'is-collapsed':''}" data-slot="${h(slot)}"><button type="button" class="slot-heading" data-collapse-slot="${h(slot)}" aria-expanded="${!collapsed}"><span class="slot-caret" aria-hidden="true">${collapsed?'›':'⌄'}</span><span>${h(cls)}</span><strong>${h(slot)}</strong><b>${items.length}</b></button><div class="card-grid">${items.map(renderCard).join('')}</div></section>`; }).join('');
}
...
function openGroupCompare(groupKey){ const rows=lastGroupedRows.filter(r=>r.Is_Dupe&&r.GroupActionKey===groupKey).map(addGrade); if(!rows.length) return setState({status:'No duplicate group found to compare.'}); document.body.classList.add('ingame-compare-ready'); openCompareModal(rows,{onTag:updateTag,onPullGroup:(groupRows,button)=>pullGroup(groupRows[0]?.GroupActionKey||groupKey,button)}); setTimeout(()=>{ if(!document.getElementById('compareOverlay') || document.getElementById('compareOverlay')?.hidden) renderInGameCompare(rows,groupKey); },0); }
function renderInGameCompare(rows,groupKey){ const overlay=document.createElement('div'); overlay.id='compareOverlay'; overlay.className='compare-overlay ingame-compare-fallback'; overlay.innerHTML=`<div class="compare-backdrop" data-close-fallback></div><section class="compare-panel"><header class="compare-head"><div><p>Duplicate Comparison</p><h2>${h(groupLabel(rows[0])||'Group')}</h2><span>${rows.length} items · ${h(rows[0]?.Slot||'Armor')}</span></div><button type="button" class="compare-close" data-close-fallback>×</button></header><div class="compare-grid">${rows.map(renderFallbackCompareCard).join('')}</div></section>`; document.body.appendChild(overlay); overlay.querySelectorAll('[data-close-fallback]').forEach(el=>el.addEventListener('click',()=>overlay.remove())); }
function renderFallbackCompareCard(row){ return `<article class="compare-card ${groupColorClass(row)}"><div class="compare-item-head"><img src="${h(row.IconUrl||row.Icon||'')}" alt=""><div><h3>${h(displayName(row))}</h3><p>${h(locationText(row))} · ${h(row.Rarity)} · ${h(row.Power||row.Light||'')}</p></div><strong class="grade-${gradeFor(row).letter}">${gradeFor(row).letter}</strong></div><div class="compare-stat-grid">${STAT_KEYS.map(k=>`<div class="compare-stat"><img src="${h(STAT_ICONS[k])}" alt="${h(STAT_LABELS[k])}"><strong>${pad(n(row[`Base${k}`]??row[k]))}</strong><span>${h(STAT_LABELS[k])}</span></div>`).join('')}${renderTotal(row)}</div></article>`; }
