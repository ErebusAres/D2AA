...
function renderArmorBonuses(row){ const perks=armorBonuses(row); if(!perks.length) return '<div class="bonus-icons is-empty"><span></span><span></span><span></span></div>'; return `<div class="bonus-icons" aria-label="Armor bonuses and perks">${perks.slice(0,5).map(renderBonusIcon).join('')}</div>`; }
function renderBonusIcon(perk){ const exotic=perk.kind==='exotic'; const icon=perk.icon?`<img src="${h(perk.icon)}" alt="">`:'✦'; return `<span class="bonus-icon ${exotic?'is-exotic':''}" tabindex="0">${icon}<span class="d2-tooltip"><b>${h(perk.name||'Armor Bonus')}</b><em>${exotic?'Exotic Perk':'Armor Bonus'}</em><p>${h(perk.description||'No description available yet.')}</p></span></span>`; }
function armorBonuses(row){ const out=[]; for(const perk of parsePerks(row.ArmorBonuses||row.ArmorPerks||row.Perks)) out.push(perk); if(row.Archetype&&row.Archetype!=='—') out.push({name:row.Archetype,description:row.ArchetypeDescription||'Armor archetype bonus.',icon:row.ArchetypeIcon,kind:'armor'}); if(String(row.Rarity).toLowerCase()==='exotic') out.push({name:row.Name||'Exotic Intrinsic',description:row.ExoticDescription||row.ExoticPerkDescription||'Exotic armor perk. Full perk details will show here when Bungie socket perk data is cached.',icon:row.ExoticIcon||row.IconUrl||row.Icon,kind:'exotic'}); const seen=new Set(); return out.filter(p=>{ const key=String(p.name||'')+String(p.description||''); if(!p.name||seen.has(key)) return false; seen.add(key); return true; }); }
