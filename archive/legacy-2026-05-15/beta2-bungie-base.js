(() => {
  ...
  const KNOWN_STAT_HASH_TO_COLUMN = {
  ...
  async function mapLimit(items, limit, worker, progress) {
    ...
    async function run() {
      while (index < items.length) {
        ...
        catch (error) { console.warn('D2AA definition failed', items[current], error); output[current] = null; }
        done++;
  ...
  function columnFromStatName(name) {
    ...
    if (!n) return null;
    if (n === 'health' || n.includes('health')) return 'Health (Base)';
    if (n === 'melee' || n.includes('melee') || n.includes('strength')) return 'Melee (Base)';
    if (n === 'grenade' || n.includes('grenade') || n.includes('discipline')) return 'Grenade (Base)';
    if (n === 'super' || n.includes('super') || n.includes('intellect')) return 'Super (Base)';
    if (n === 'class' || n.includes('class ability') || n === 'mobility') return 'Class (Base)';
    if (n === 'weapons' || n === 'weapon' || n.includes('weapon')) return 'Weapons (Base)';
    return null;
  }

  async function getStatColumnMap(hashes) {
    const unique = [...new Set(hashes.map(Number).filter(Boolean))];
    const result = { ...KNOWN_STAT_HASH_TO_COLUMN };
    const unknown = unique.filter((hash) => !result[hash] && !STAT_MEMORY.has(hash));
    if (unknown.length) {
      setStatus(`Resolving stat definitions: 0/${unknown.length}`, false);
      const defs = await mapLimit(unknown, 8, (hash) => getDef('DestinyStatDefinition', hash), (done, total) => setStatus(`Resolving stat definitions: ${done}/${total}`, false));
      unknown.forEach((hash, i) => STAT_MEMORY.set(hash, defs[i] || null));
    }
    for (const hash of unique) {
      if (result[hash]) continue;
      const def = STAT_MEMORY.get(hash);
      const column = columnFromStatName(def?.displayProperties?.name || def?.statName || '');
      if (column) result[hash] = column;
    }
    return result;
  }
  ...
  function socketBonusTotals(plugDefs, statColumnMap) {
    ...
    for (const plugDef of plugDefs || []) {
      for (const stat of plugDef?.investmentStats || []) {
        const column = statColumnMap[Number(stat.statTypeHash)];
  ...
  function statsForItem(def, statComponent, socketBonusRow, statColumnMap) {
    ...
    for (const [hash, stat] of Object.entries(source)) {
      const column = statColumnMap[Number(hash)];
      if (!column) continue;
  ...
  async function importArmorBase() {
    ...
    const defsList = await mapLimit(uniqueItemHashes, 8, (hash) => getDef('DestinyInventoryItemDefinition', hash), (done, total) => setStatus(`Resolving item definitions: ${done}/${total}`, false));
    const itemDefs = Object.fromEntries(uniqueItemHashes.map((hash, i) => [hash, defsList[i]]));

    const armorItems = allItems.filter((item) => item.itemInstanceId && isArmorDef(itemDefs[item.itemHash]));
    ...
    setStatus(`Resolving socket plug definitions: 0/${uniquePlugHashes.length}`, false);
    const plugDefsList = await mapLimit(uniquePlugHashes, 8, (hash) => getDef('DestinyInventoryItemDefinition', hash), (done, total) => setStatus(`Resolving socket plug definitions: ${done}/${total}`, false));
    ...
    for (const plugDef of plugDefsList.filter(Boolean)) {
      for (const stat of plugDef.investmentStats || []) statHashes.push(stat.statTypeHash);
    }
    const statColumnMap = await getStatColumnMap(statHashes);
    ...
    setStatus(`Bungie sync complete: ${rows.length} armor items in ${seconds}s. Base stats derived from Bungie stat definitions + socket bonuses.`, true);
  }

  const btn = $('bungieImportBaseBtn');
  if (btn) {
    btn.addEventListener('click', (event) => {
      event.preventDefault();
      event.stopImmediatePropagation();
      importArmorBase().catch((error) => { console.error(error); setStatus(error.message || String(error), false); });
    }, true);
  }
})();
