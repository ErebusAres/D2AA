(() => {
  ...
  function renderGridCard(row) {
    const icon = itemIcon(row); const fallback = slotLabel(row.Type).slice(0, 1).toUpperCase(); const gColor = groupColor(row); const style = gColor ? ` style="--group-glow:${gColor}"` : ''; const slot = slotLabel(row.Type); const actionDisabled = row.Source === 'Bungie' && row.IsEquipped ? ' disabled' : ''; const groupText = row.Is_Dupe ? `${row.Dupe_Group}` : `${row.Rarity || 'Unknown'}`; const light = lightLevel(row);
    return `<article class="grid-card ${rarityClass(row.Rarity)}${row.Is_Dupe ? ' is-dupe' : ''}" data-grid-id="${escapeHtml(normId(row.Id))}"${style}><div class="grid-card-top"><div class="grid-item-icon">${icon ? `<img src="${escapeHtml(icon)}" alt="" loading="lazy">` : `<span>${fallback}</span>`}${light ? `<span class="grid-light" title="Light / Power level">${light}</span>` : ''}</div><div class="grid-item-title"><div class="grid-item-name" title="${escapeHtml(row.Name)}">${escapeHtml(row.Name || '(Unnamed item)')}</div><div class="grid-icon-row">${img(RARITY_ICONS[row.Rarity], row.Rarity || 'Rarity')}${mask(SLOT_ICONS[slot], slot)}${mask(CLASS_ICONS[row.Equippable], row.Equippable)}<span class="grid-location" title="${escapeHtml(locationLabel(row))}">${locationIcon(row)}</span></div></div><button class="grid-tag" type="button" data-grid-action="tag" title="${escapeHtml(tagLabel(row.Tag))} — change tag">${tagEmoji(row.Tag)}</button></div><div class="grid-body"><div class="grid-primary-row"><span class="grid-total" title="Base stat total">${num(row['Total (Base)'])}</span><span class="grid-tier" title="Tier ${num(row.Tier)}">${tierDiamonds(row)}</span><span class="grid-rank" title="Rank">${escapeHtml(row.Rank || '')}</span></div>${renderStats(row)}<div class="grid-group-row"><span class="grid-group" title="${escapeHtml(row.Is_Dupe ? `${slot} duplicate group ${row.Dupe_Group}` : `${row.Rarity || 'Unknown'} solo item`)}">${row.Is_Dupe ? `${mask(SLOT_ICONS[slot], slot, 'grid-group-slot-icon')}` : ''}<span>${escapeHtml(groupText)}</span></span>${light ? `<span class="grid-power-pill" title="Light / Power level">✦ ${light}</span>` : ''}</div></div><div class="grid-actions${row.Is_Dupe ? '' : ' grid-actions--single'}"><button class="grid-action" type="button" data-grid-action="primary"${actionDisabled}>${buttonText(row)}</button>${renderGroupButton(row, slot)}</div></article>`;
  }
