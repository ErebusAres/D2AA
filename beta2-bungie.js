(() => {
  ...
  const STORAGE = { config: 'd2aa_bungie_public_config_v1', token: 'd2aa_bungie_token_v1', state: 'd2aa_bungie_oauth_state_v1', manifest: 'd2aa_bungie_manifest_v1', defs: 'd2aa_bungie_defs_v1' };
  ...
  function refreshStatus() {
    const cfg = getConfig(); const token = getToken();
  ...
  function setupConfig() {
    ...
    writeJson(STORAGE.config, { apiKey: apiKey.trim(), clientId: clientId.trim(), redirectUri: redirectUri.trim() }); refreshStatus();
  }
  ...
  function login() {
    const cfg = getConfig(); if (!cfg.apiKey || !cfg.clientId || !cfg.redirectUri) { setupConfig(); return; }
    const state = randomState(); localStorage.setItem(STORAGE.state, state);
    const url = new URL(AUTH_URL); url.searchParams.set('client_id', cfg.clientId); url.searchParams.set('response_type', 'code'); url.searchParams.set('state', state); window.location.href = url.toString();
  }
  async function exchangeCode(code) {
    const cfg = getConfig(); const body = new URLSearchParams(); body.set('grant_type', 'authorization_code'); body.set('code', code); body.set('client_id', cfg.clientId);
    ...
    const json = await res.json().catch(() => ({})); if (!res.ok) throw new Error(json.error_description || json.Message || `OAuth token exchange failed (${res.status}).`); saveToken(json);
  }
  async function refreshToken() {
    const cfg = getConfig(); const token = getToken(); if (!token.refresh_token) throw new Error('No refresh token found. Connect your Destiny account again.');
    const body = new URLSearchParams(); body.set('grant_type', 'refresh_token'); body.set('refresh_token', token.refresh_token); body.set('client_id', cfg.clientId);
    const res = await fetch(TOKEN_URL, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'X-API-Key': cfg.apiKey }, body });
    const json = await res.json().catch(() => ({})); if (!res.ok) throw new Error(json.error_description || json.Message || `OAuth refresh failed (${res.status}).`); return saveToken(json);
  }
  async function handleOAuthRedirect() {
    const url = new URL(window.location.href); const code = url.searchParams.get('code'); if (!code) return;
    const returnedState = url.searchParams.get('state'); const expectedState = localStorage.getItem(STORAGE.state);
    if (expectedState && returnedState !== expectedState) { setStatus('Bungie sign-in failed: OAuth state mismatch.', false); return; }
    try { setStatus('Completing Bungie sign-in...', false); await exchangeCode(code); localStorage.removeItem(STORAGE.state); url.searchParams.delete('code'); url.searchParams.delete('state'); history.replaceState({}, document.title, url.toString()); refreshStatus(); }
    catch (error) { console.error(error); setStatus(error.message, false); }
  }
  async function bungieFetch(path, auth = false, options = {}) {
    const cfg = getConfig(); if (!cfg.apiKey) throw new Error('Missing Bungie API key.');
    ...
    const res = await fetch(`${API_ROOT}${path}`, { ...options, headers }); const json = await res.json().catch(() => ({}));
    if (!res.ok || (json.ErrorCode && json.ErrorCode !== 1)) throw new Error(json.Message || `Bungie request failed (${res.status}).`); return json.Response;
  }
  ...
  async function getMembership() {
    const data = await bungieFetch('/User/GetMembershipsForCurrentUser/', true); const memberships = data.destinyMemberships || [];
    const primary = memberships.find((m) => m.membershipId === data.primaryMembershipId) || memberships[0]; if (!primary) throw new Error('No Destiny membership found for this Bungie account.'); return primary;
  }
  ...
  async function getDefinitionTable(tableName) {
    if (DEF_MEMORY[tableName]) return DEF_MEMORY[tableName]; const manifest = await getManifest(); const path = manifest.jsonWorldComponentContentPaths?.en?.[tableName];
    if (!path) throw new Error(`Manifest table missing: ${tableName}.`); const res = await fetch(`https://www.bungie.net${path}`, { cache: 'force-cache' });
    if (!res.ok) throw new Error(`Manifest download failed: ${tableName} (${res.status}).`); const table = await res.json(); DEF_MEMORY[tableName] = table; return table;
  }
  async function getDefinitions() {
    const [items, buckets, tiers, stats] = await Promise.all([getDefinitionTable('DestinyInventoryItemDefinition'), getDefinitionTable('DestinyInventoryBucketDefinition'), getDefinitionTable('DestinyItemTierTypeDefinition'), getDefinitionTable('DestinyStatDefinition')]);
  ...
  function statValue(stat) { return Number(stat?.value ?? stat?.statValue ?? stat?.base ?? stat?.minimum ?? 0); }
  function socketStatBonuses(socketComponent, itemDefs, statDefs) {
    ...
    for (const socket of socketComponent?.sockets || []) {
      ...
      for (const stat of plugDef.investmentStats || []) {
        const column = statColumnForHash(stat.statTypeHash, statDefs);
  ...
  function statsForItem(item, def, statComponent, socketComponent, statDefs, itemDefs) {
    ...
    const bonuses = socketStatBonuses(socketComponent, itemDefs, statDefs);
    const rowStats = emptyArmorStats();
    for (const [hash, stat] of Object.entries(source)) {
      const column = statColumnForHash(hash, statDefs); if (!column) continue;
      const current = statValue(stat);
      const base = Math.max(0, current - Number(bonuses[column] || 0));
      rowStats[column] = base;
    }
    rowStats['Total (Base)'] = ARMOR_STAT_KEYS.reduce((sum, key) => sum + Number(rowStats[key] || 0), 0);
    rowStats['Total (Current)'] = ARMOR_STAT_KEYS.reduce((sum, key) => sum + Number(statValue(source[Object.keys(source).find((hash) => statColumnForHash(hash, statDefs) === key)]) || 0), 0);
    return rowStats;
  }
  function armorTier(total) { return Math.max(0, Math.floor(Number(total || 0) / 10)); }
  function buildCharacterMap(profile) {
    const map = {}; for (const [characterId, character] of Object.entries(profile.characters?.data || {})) { const className = CLASS_TYPE[character.classType] || 'Unknown'; map[className] = { characterId, className }; } return map;
  }
  function normalizeArmor(profile, defs, membership) {
    ...
    const characterMap = buildCharacterMap(profile); const seen = new Set(); const rows = [];
    for (const item of collectItems(profile)) {
      const instanceId = item.itemInstanceId; if (!instanceId || seen.has(instanceId)) continue; seen.add(instanceId);
      const def = defs.items[item.itemHash]; if (!def || def.itemType !== 2) continue;
      const slot = slotForItem(def, defs.buckets); if (!slot) continue;
      const equippable = CLASS_TYPE[def.classType]; if (!equippable) continue;
      const rarity = rarityForItem(def, defs.tiers); if (!['Common', 'Uncommon', 'Rare', 'Legendary', 'Exotic'].includes(rarity)) continue;
      const type = slot === 'Class Item' ? CLASS_ITEM_BY_CLASS[equippable] : slot;
      const statRow = statsForItem(item, def, statComponents[instanceId], socketComponents[instanceId], defs.stats, defs.items);
  ...
  async function importArmor() {
    try {
      ...
      setStatus('Fetching Bungie profile, sockets, and manifest definitions...', false);
      ...
      const defsPromise = getDefinitions(); const [profile, defs] = await Promise.all([profilePromise, defsPromise]);
      const rows = normalizeArmor(profile, defs, membership); if (!rows.length) throw new Error('Bungie sync completed, but no armor items were found.');
      LAST_CONTEXT = { membership, profile, defs }; window.D2AA?.loadRows?.(rows, `Bungie sync • ${rows.length} armor items`); setStatus(`Bungie sync complete: ${rows.length} armor items. Base stats derived by subtracting socket stat bonuses.`, true);
    } catch (error) { console.error(error); setStatus(error.message, false); }
  }
  async function pullItem(row) {
    ...
    await bungiePost('/Destiny2/Actions/Items/TransferItem/', { itemReferenceHash: Number(row.ItemHash), stackSize: 1, transferToVault: false, itemId: row.Id, characterId: row.TargetCharacterId, membershipType: Number(row.MembershipType) }); return 'Pulled';
  }
  async function vaultItem(row) {
    if (!row || row.Source !== 'Bungie') throw new Error('This action requires a Bungie-synced item.');
    if (row.IsInVault) return 'Already in vault'; if (row.IsEquipped) throw new Error('Equipped items must be unequipped before they can be moved to the vault.'); if (!row.OwnerCharacterId) throw new Error('Missing source character for this item. Sync from Bungie again.');
    await bungiePost('/Destiny2/Actions/Items/TransferItem/', { itemReferenceHash: Number(row.ItemHash), stackSize: 1, transferToVault: true, itemId: row.Id, characterId: row.OwnerCharacterId, membershipType: Number(row.MembershipType) }); return 'Vaulted';
  }
  async function pullItems(rows) {
    const bungieRows = (rows || []).filter((row) => row?.Source === 'Bungie'); if (!bungieRows.length) throw new Error('No Bungie-synced items selected.');
    let done = 0; for (const row of bungieRows) { setStatus(`Pulling ${++done}/${bungieRows.length}: ${row.Name}`, false); await pullItem(row); }
  ...
  $('bungieSetupBtn')?.addEventListener('click', setupConfig); $('bungieLoginBtn')?.addEventListener('click', login); $('bungieImportBtn')?.addEventListener('click', importArmor); refreshStatus(); handleOAuthRedirect();
})();
