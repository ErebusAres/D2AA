(() => {
  ...
  function tokenIsValid(token = getToken()) {
  ...
  function refreshStatus() {
    ...
    if (!cfg.apiKey || !cfg.clientId || !cfg.redirectUri) {
      ...
      setStatus('Bungie API configured. Sign in to authorize inventory import.', false);
    } else {
      setStatus('Bungie API ready. Import armor when ready.', true);
    }
  }
  ...
  function login() {
    let cfg = getConfig();
    if (!cfg.apiKey || !cfg.clientId || !cfg.redirectUri) setupConfig();
    cfg = getConfig();
    if (!cfg.apiKey || !cfg.clientId || !cfg.redirectUri) return;
  ...
  async function exchangeCode(code) {
    ...
    const res = await fetch(TOKEN_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'X-API-Key': cfg.apiKey },
      body
    });
  ...
  async function refreshToken() {
    const cfg = getConfig();
    const token = getToken();
    if (!token.refresh_token) throw new Error('No refresh token found. Sign in with Bungie again.');
  ...
  async function handleOAuthRedirect() {
    ...
    if (expectedState && returnedState !== expectedState) {
      setStatus('Bungie sign-in failed: OAuth state mismatch.', false);
      return;
    }
  ...
  async function bungieFetch(path, auth = false) {
    const cfg = getConfig();
    if (!cfg.apiKey) throw new Error('Missing Bungie API key. Click Bungie API Setup first.');
    ...
    if (!res.ok || (json.ErrorCode && json.ErrorCode !== 1)) {
      throw new Error(json.Message || `Bungie request failed (${res.status}).`);
    }
  ...
  async function getManifest() {
    const cached = readJson(STORAGE.manifest, null);
    if (cached?.jsonWorldComponentContentPaths?.en) return cached;
    const manifest = await bungieFetch('/Destiny2/Manifest/');
    writeJson(STORAGE.manifest, manifest);
    return manifest;
  }
  ...
  function collectItems(profile) {
    const out = [];
    if (profile.profileInventory?.data?.items) out.push(...profile.profileInventory.data.items);
    for (const container of Object.values(profile.characterInventories?.data || {})) {
      if (container.items) out.push(...container.items);
    }
    for (const container of Object.values(profile.characterEquipment?.data || {})) {
      if (container.items) out.push(...container.items);
    }
    return out;
  }

  function statColumnForHash(hash, statDefs) {
    const name = statDefs?.[hash]?.displayProperties?.name || '';
    return STAT_NAME_MAP[name.trim().toLowerCase()] || null;
  }

  function slotForItem(def, bucketDefs) {
    const bucketName = bucketDefs?.[def.inventory?.bucketTypeHash]?.displayProperties?.name || '';
    return SLOT_BY_BUCKET_NAME[bucketName] || SLOT_BY_BUCKET_NAME[def.itemTypeDisplayName] || null;
  }

  function rarityForItem(def, tierDefs) {
    return tierDefs?.[def.inventory?.tierTypeHash]?.displayProperties?.name || def.inventory?.tierTypeName || 'Unknown';
  }
  ...
  function statsForItem(item, def, statComponent, statDefs) {
    ...
    for (const [hash, stat] of Object.entries(source)) {
      ...
      if (!column) continue;
      rowStats[column] = Number(stat.value ?? stat.statValue ?? 0);
    }
  ...
  function normalizeArmor(profile, defs) {
    ...
    for (const item of collectItems(profile)) {
      ...
      rows.push({
        ...
        Tag: '',
        Tier: rarity === 'Exotic' ? 6 : rarity === 'Legendary' ? 5 : rarity === 'Rare' ? 4 : rarity === 'Uncommon' ? 3 : 2,
        ...statRow
      });
    }
  ...
  async function importArmor() {
    try {
      ...
      const rows = normalizeArmor(profile, defs);
      if (!rows.length) throw new Error('Bungie import completed, but no armor items were found.');
      window.D2AA?.loadRows?.(rows, `Bungie import • ${rows.length} armor items`);
      setStatus(`Bungie import complete: ${rows.length} armor items.`, true);
    } catch (error) {
      console.error(error);
      setStatus(error.message, false);
    }
